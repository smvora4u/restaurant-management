...
import { toTimestamp, formatFullDateTime } from '../../utils/dateFormatting';
import {
...
/** Format order date for receipts - uses order time only, never print time. Uses project dateFormatting. */
function formatOrderDate(order: BillPrintOrder): string {
  const val = order?.createdAt ?? (order as any)?.created_at;
  if (val == null || val === '') return 'Order date unavailable';
  const ts = typeof val === 'object' && val !== null && '$date' in val
    ? toTimestamp((val as { $date: string }).$date)
    : toTimestamp(val);
  return ts !== null ? formatFullDateTime(ts) : 'Order date unavailable';
}
...
/**
 * Encodes KOT (Kitchen Order Ticket) to ESC/POS bytes - items only, no prices.
 */
...
/**
 * Fallback: opens a print window with HTML KOT and calls window.print().
 */
function printViaBrowserKOT(
  order: BillPrintOrder,
  restaurant: BillPrintRestaurant,
  menuItems: BillPrintMenuItem[],
  autoPrint: boolean
...
<head>
  <meta charset="utf-8">
  <title>KOT - Order #${order.id.slice(-8)}</title>
  <style>
...
<body>
  <div class="receipt">
    ...
    <div class="divider"></div>
    <div class="center" style="margin-top: 12px; font-size: 10px;">KITCHEN</div>
  </div>
</body>
</html>`;
...
export interface PrintKOTOptions {
  /** When provided and restaurant has networkPrinter configured, tries network print first */
  requestNetworkKOT?: (orderId: string) => Promise<boolean>;
}
...
/**
 * Hybrid print for KOT: tries network printer (if configured), then direct USB/Serial, then browser print dialog.
 */
export function printKOT(
  ...
  menuItems: BillPrintMenuItem[],
  autoPrint = true,
  options?: PrintKOTOptions
): void {
  const encodedBytes = encodeKOTToEscPos(order, restaurant, menuItems);

  if (options?.requestNetworkKOT) {
    options.requestNetworkKOT(order.id).then((ok) => {
      if (ok) return;
      tryLocalOrBrowser();
    }).catch(() => tryLocalOrBrowser());
    return;
  ...
  function tryLocalOrBrowser() {
    if (isDirectPrintSupported()) {
      ...
      if (state.isConnected) {
        printReceipt(encodedBytes).then((ok) => {
          if (!ok) {
            printViaBrowserKOT(order, restaurant, menuItems, autoPrint);
          }
        });
        return;
      }
